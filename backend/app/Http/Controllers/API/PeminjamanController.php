...
{ 
    ...
    'pengembalian']); 
            ...
            $peminjaman = request()->has('page') || request()->has('per_page') ? $query->latest()->paginate($perPage)->withQueryString() : $query->latest()->get(); 
    ...
    { 
        try { 
            $peminjaman = DB::transaction(function () use ($request) { 
                ...
                foreach ($request->items as $item) {// lockForUpdate mengunci baris data di database sampai transaksi ini COMMIT 
                    $alat = Alat::lockForUpdate()->findOrFail($item['alat_id']);if ($alat->stok < $item['jumlah']) { 
    ...
    { 
        ...
        try { 
            DB::transaction(function () use ($peminjaman) { 
                ...
                foreach ($peminjaman->detailPinjam as $detail) {// Mengunci baris alat demi validasi final sebelum stok dikurangi 
