...
{ 
    ...
    { 
        ...
        $alat = request()->has('page') || request()->has('per_page')
            ? Alat::with('kategori')->latest()->paginate($perPage)->withQueryString()
            : Alat::with('kategori')->latest()->get();        
    ...
    { 
        $perPage = request()->input('per_page', 15);
        $alat = request()->has('page') || request()->has('per_page')
            ? Alat::with('kategori')->tersedia()->latest()->paginate($perPage)->withQueryString()
            : Alat::with('kategori')->tersedia()->latest()->get(); 
        return response()->json([ 
