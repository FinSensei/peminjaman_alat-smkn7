    ...
    { 
        try { 
            $pengembalian = DB::transaction(function () use ($request) { 
                ...
                $hariTelat = $hariIni->greaterThan($tglKembaliPlan) ? $hariIni->diffInDays($tglKembaliPlan) : 0;
                $statusPeminjamanBaru = $hariTelat > 0 ? 'telat' : 'dikembalikan';
                $dendaPerHari = config('inventory.denda_per_hari', 5000);
                $dendaAuto = $hariTelat * $dendaPerHari;
 
                // 1. Insert data ke tabel pengembalian (auto denda jika tidak diisi manual)
                ...
                foreach ($peminjaman->detailPinjam as $detail) { 
                    ...
                    // increment() otomatis menambah nilai pada field  ditentukan 
    ...
    { 
        try { 
            DB::transaction(function () use ($pengembalian) { 
                ...
                foreach ($peminjaman->detailPinjam as $detail) {$alat = Alat::lockForUpdate()->findOrFail($detail->alat_id); 
